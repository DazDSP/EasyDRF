...
bool reedSolomonCoder::decode(char*& ba, char* fn, char*& newFileName, char*& baFile, char* extension, int*& erasuresArray)
{
...
void reedSolomonCoder::distribute(char*src, char*dst, int rows, int cols, int reverse)
{
  ...
  for(i=0;i<bep_size;i++)
    {
      ...
      int failure=rsd32(((char *)ec_buf.data()+(i*rs_bsize)),eras_pos, nr_erasures);
      if (failure>0)
        {
...
bool reedSolomonCoder::encode(char*&ba, char* extension,eRSType rsType)
{
  int i,j;
  unsigned char dataByte;
  char* temp;
  char* suffix=extension.leftJustified(3,0);
  tr_buf=ba;
  fileType=rsType;
  rs_bsize=RSBSIZE;
  switch (fileType)
